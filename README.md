...
V1 的多来源、摘要、正式 `/discord` 阅读页、定时补采和公开 fixture 验收已完成：114 条 pgTAP、49 个控制面测试、60 个 Worker 测试和 3 个 V1 E2E 测试通过。2026-07-20 已部署到专用 V1 环境，两个真实授权来源各完成一个有界单页 history 任务并持久化成功。真实增量二次去重/checkpoint、可操作失败隔离、普通用户阅读、桌面/手机视觉和真实质量抽检仍待完成，所以当前仍是**条件验收**，不能称为正式可用或生产 SLA。
...
- [docs/spikes/2026-07-19-v1-decision-report.md](docs/spikes/2026-07-19-v1-decision-report.md)：V1 条件 Final Report 与远程/真实验收门槛。
...
V1 的专用部署和真实双来源有界 history 已完成；下一步是在继续授权下补齐真实增量/二次去重/checkpoint、失败隔离和普通用户阅读验收。V2 X、V3 收敛以及模块 2–4 仍不在当前范围。
