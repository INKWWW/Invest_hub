...
V1 的多来源、摘要、正式 `/discord` 阅读页、定时补采和公开 fixture 验收已完成：114 条 pgTAP、49 个控制面测试、59 个 Worker 测试和 3 个 V1 E2E 测试通过。它尚未部署到专用 V1 环境，也未完成真实双来源验收，所以当前是**条件验收**，不能称为正式可用或生产 SLA。
...
V1 的本地实现已完成；下一步是在专用 V1 环境完成部署与远程合成验收，并在明确授权后完成真实双来源 Discord 验收。V2 X、V3 收敛以及模块 2–4 仍不在当前范围。
