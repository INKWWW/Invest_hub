-- A scheduled batch owns the task that collected it.  A manual recovery batch
-- deliberately reuses that immutable, successful task as frozen input, but
-- only when the source and exact cutoff still agree.
  ...
  if new.x_sync_task_id is not null then
    ...
    if v_batch.scheduled_window_key ~ '^manual:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then
      if v_task.status <> 'succeeded'
         ...
         or v_task.collection_batch_id is null
