...
begin
  ...
  -- Union this tick's work with the bounded authoritative frontier for each
  -- assigned source.  There is no time cutoff: a long-stopped Worker still
  -- receives its oldest active backlog, while completed history is bounded to
  -- the exact task named by each source coverage row.
...
revoke all on function public.enqueue_due_x_tasks(uuid, timestamptz) from public, anon, authenticated;
