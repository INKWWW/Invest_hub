-- A failed window whose end is at or before the current coverage waterline is
-- retained as audit history, not pending work.  It therefore must not prevent
-- the current waterline window from completing.  Uncovered failed predecessors
-- still block completion exactly as before.

create or replace function public.complete_windowed_capture_range_v2_x_core(
  p_task_id uuid,
  p_attempt integer,
  p_worker_id uuid,
  p_payload jsonb
...
begin
  ...
  if exists (
    ...
    where predecessor.source_id = v_task.source_id and predecessor.id <> v_task.id
      ...
      and (
        v_coverage.coverage_through_at is null
        or (predecessor.capture_range->>'end_at')::timestamptz > v_coverage.coverage_through_at
      )
  ) then
