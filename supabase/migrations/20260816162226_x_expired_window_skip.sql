...
create or replace function public.reap_expired_x_window_tasks(p_now timestamptz)
returns integer
language plpgsql
security definer
...
begin
  ...
  loop
    perform pg_advisory_xact_lock(hashtextextended(v_task.source_id::text, 24005));
...
as $$
begin
  perform public.reap_expired_x_window_tasks(p_now);
  return public.claim_next_task_v2_base_without_lease_reap(p_worker_id, p_now);
end;
$$;

revoke all on function public.reap_expired_x_window_tasks(timestamptz)
