-- Bounded X history has the same page-evidence durability requirement as a
-- scheduled window.  It is deliberately limited to tasks whose immutable
-- capture range is also history, so legacy page-capped history remains on its
-- existing path.
...
begin
  ...
  if not found
     or v_task.collection_scope->>'mode' not in ('window', 'history')
     or v_task.capture_range->>'mode' is distinct from v_task.collection_scope->>'mode'
  ...
  select * into v_existing from public.sync_task_capture_segments
  where task_id = p_task_id and idempotency_key = p_segment->>'idempotency_key' for update;
  if found then
