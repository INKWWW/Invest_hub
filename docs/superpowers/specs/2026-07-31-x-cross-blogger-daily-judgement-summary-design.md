...
- 状态：**已批准（用户确认 2026-07-31）；2026-08-01 用户确认补偿性再生成语义。**
- 日期：2026-07-31
...
每份总结保存输入窗口段、分析和来源身份的不可变引用，以及输出的 schema/prompt/provider 遥测。Provider 在尚无成功版本时失败，只重试同一 run；其后首次成功写入 `revision 1`，不会把失败尝试伪装成 Reader 版本。已成功批次只有经过管理员授权的显式 **regeneration** 才可创建一个新的独立 run；该 run 成功时为同一采集批次追加递增的**总结修订版本**（例如 `revision 2`），不覆盖旧文本。regeneration 不得重新采集、修改来源快照、改变 coverage 或 checkpoint；Reader 默认显示该批次最新修订，同时允许查看较早修订及其覆盖范围。不同采集批次按 `cutoff_at` 倒序显示，最近批次始终在最上方。
...
6. Provider 在尚无成功版本时的重试成功创建 `revision 1`；已成功批次经管理员显式 regeneration 成功时创建递增的新修订版本，旧版本不被覆盖；Reader 默认显示最新修订，历史可追溯。
