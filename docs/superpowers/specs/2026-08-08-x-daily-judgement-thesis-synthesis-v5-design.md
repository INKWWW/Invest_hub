# X 每日判断完整主线与 AI 综合研判 v5 Spec

## 文档状态

- 阶段：V2 受控生产试运行的阅读与理解能力修订
- 状态：**待书面复核；方案 B、内容架构、v5 合同边界和页面线框已由用户于 2026-08-08 确认**
- 日期：2026-08-08
...
每日判断还新增“AI 综合研判”：只基于本批次已经纳入且可追溯的完整判断，按语义相关性输出零到多条独立 AI 判断，说明跨博主共性、关键矛盾和需要继续观察的变量。AI 判断不要求覆盖全部博主或全部主题，不允许把无关内容强行融合为一个总判断。
...
AI 综合研判不得调用外部知识、模型记忆、网页检索或未提供事实，不得生成系统自己的买入、卖出、加仓、减仓、建仓、清仓、抄底或追涨建议。允许提出的“观察建议”仅限于从已纳入观点的条件、不确定性和分歧中提炼需要继续验证的变量。

## 3. 方案选择与职责边界

本设计采用新的 v5 分层合同，而不是只修改 v4 Prompt，也不增加第二次生产模型调用。

| 层级 | 版本与职责 | 本次是否改变 |
| --- | --- | --- |
| 单帖分析 | `v4-x-post-analysis`：忠实提取一条帖子中的观点、条件、论据、操作表述和证据 | 否 |
| 单博主窗口 | `v4-x-window`：整理同一博主、同一完成窗口内的原子观点 | 否 |
| 跨博主每日判断 | `v5-x-cross-blogger`：聚类原子证据、重建完整判断主线、形成零到多条 AI 判断 | 是 |
| Reader | v2/v3/v4 保持历史兼容；v5 使用新的主线与 AI 判断布局 | 是 |
...
三类 `theses` 继续保留“个股与产业”“市场结构”“投资策略与心态”的分类边界，但数组项不再代表孤立事实，而代表一条完整判断主线。顶层 `uncertainties` 只承载批次整体覆盖或输入限制，不重复单条主线的不确定性。
...
`ai_judgements` 是零到多条独立 AI 判断，不存在强制覆盖所有内容的全局 `overview`。每条结构为：

```json
{
  "ai_judgement_id": "ai-01",
  "headline": "独立 AI 判断标题",
  "synthesis": "仅基于相关完整主线形成的二阶研判",
  "common_points": ["跨博主共性"],
  "conflict_points": ["方向、假设、条件或时间尺度上的矛盾"],
  "watch_variables": ["可验证或推翻相关判断的输入内变量"],
  "related_thesis_ids": ["security-01", "market-01"],
  "uncertainties": ["该 AI 判断的覆盖和推理限制"]
}
```

`ai_judgement_id` 固定使用 `ai-01`、`ai-02` 等连续编号，在当前输出内唯一，只用于内部关联和稳定渲染，不向普通读者展示。

每条 AI 判断只组合语义上真正相关的 thesis，可以只涉及部分博主和部分主题。不同标的、产业、核心问题或时间尺度原则上分开；只有输入中的判断本身建立了明确比较或共同因果关系时，才允许跨对象综合。

AI 判断必须带来二阶价值，不能只是换句话复述一条单来源主线。每条 AI 判断所引用 thesis 的支持与反对来源并集必须至少包含两位独立博主；可以引用一条已经聚合多位博主的 thesis，也可以引用多条相关 thesis。若没有满足条件的跨博主关系，则 `ai_judgements` 必须为空，Reader 不显示空模块。

`common_points`、`conflict_points` 和 `watch_variables` 均允许为空，但三者至少一项必须非空；不能为了填满固定版式而虚构共性、分歧或观察变量。AI 判断数量没有机械上限，Prompt 应要求输出覆盖真实独立主题所需的最小集合，避免一个“大一统判断”，也避免将同一 AI 判断再次碎片化。
...
flowchart TD
  ...
  B["冻结的 v4 单博主窗口观点"] --> C
  C --> D["同一模型调用内：聚类、重建主线、形成 AI 判断、静默自检"]
  D --> E["结构、证据、归因和安全校验"]
  E --> F["追加式 v5 判断版本"]
  F --> G["Reader：AI 综合研判 → 分类完整主线 → 单个博主观点"]
```

本次不新增第二次生产模型调用或运行时语义聚类器。Worker 继续向一次 Provider 调用提供冻结的 included sources、v4 窗口输出和 v4 单帖分析；Prompt 在同一调用内完成聚类与综合，程序不根据文字相似度自行合并模型输出。
...
- 顶层、thesis、情景分支、博主操作和 AI 判断字段精确，无缺失或额外字段；版本必须为 v5。
- thesis 与 AI 判断 ID 格式、类别前缀、连续编号、唯一性和引用闭包正确。
...
- `related_thesis_ids` 必须非空、唯一并引用当前输出中的 thesis；其来源并集至少包含两位独立博主。
- excluded sources 和无新增来源只能形成覆盖说明，不能成为 thesis 或 AI 判断证据。
- 强共识措辞仍要求至少两位独立支持来源且没有反对或保留来源。
- 自然语言字段不得泄露 source、analysis、post、batch、run 或 segment 等内部 ID，不得包含系统买卖指令或不安全运行信息。
...
- 多位博主使用不同措辞表达同一命题时应形成共性；
- 同一命题的相反立场应作为分歧保留；
- 无关标的或不同时间尺度不得错误合并；
- 单来源复述不得生成 AI 判断；
- 没有真实跨博主关系时 `ai_judgements` 应为空；
- excluded source、提示注入、条件升级为确定事实和输入外知识必须失败。

Eval 顺序固定为确定性合同检查，再使用受约束的语义 Judge 检查碎片化、错误合并、信息丢失和不受支持的 AI 推理；严重或不确定结果进入人工复核。Judge 只用于发布前评估，不成为第二次生产调用，也不能覆盖确定性证据、安全或归因失败。
...
AI 综合研判置于分类主线之前，使用独立的低饱和样式，并固定显示“仅基于本批次已纳入的博主观点，不使用外部知识，不构成交易建议”。多条 AI 判断纵向排列；不存在一个强行覆盖所有内容的总览。未参与 AI 综合但仍有价值的 thesis 保留在下方分类中，不因未被引用而隐藏。

每条 thesis 依次展示标题、连续综合阐述、可选情景分支、按博主归因的操作表述、支持/不同观点和不确定性。情景分支作为所属主线内部的“情景 A/B/C”显示，不使用新的“判断主线”编号。类别为空时继续隐藏；AI 判断为空时整个 AI 模块隐藏，不显示凑数文案。

现有日期、博主筛选、批次倒序、最新批次默认展开、历史修订折叠、单博主分块和最新窗口优先行为保持不变。跨博主判断及 AI 综合研判仍只在“全部博主”视图显示；单博主筛选不能裁剪或伪造跨博主结果。桌面端和 375px 移动端保持同一纵向语义顺序。

Reader-safe DTO 只映射可读标题、阐述、情景、操作显示、博主显示名和不确定性；不得输出 thesis/AI 内部关联 ID、source/analysis/post ID、原始正文、Prompt、Provider、任务诊断、Cookie、Profile 或本地路径。

## 9. 失败处理、兼容性与发布边界

- `no_new_information` 批次继续不调用 Provider；有新增但没有可验证投资判断时允许三类 thesis 和 AI 判断全部为空，并准确保留覆盖说明。
...
- partial 批次可以生成 v5，但 AI 判断必须继承批次覆盖限制；未纳入来源不能被解释为没有反对意见。
...
5. AI 综合研判可输出零到多条独立判断，只关联语义相关的 thesis，不要求覆盖全部博主和全部内容，也不存在强制全局 overview。
6. 每条 AI 判断的相关 thesis 来源并集至少包含两位独立博主，并至少包含一项真实共性、分歧或观察变量；单来源复述、无二阶价值和输入外推理被拒绝。
7. AI 判断不引入输入外事实、数字、标的、事件或系统交易建议；观察变量只能来自输入条件、不确定性和分歧。
8. thesis、情景、博主操作和 AI 关联的来源、analysis、evidence 与冻结批次同源且闭合；excluded sources 不能成为判断证据。
9. v5 校验失败不形成当前 Reader 版本，不影响单来源采集与既有成功数据；v2/v3/v4 历史记录保持不可变且可读。
10. Reader 按“覆盖 → AI 综合研判 → 三类完整判断主线 → 单个博主观点”阅读顺序展示；AI 和空类别按规则隐藏，情景嵌套、博主归因和桌面/375px 阅读均清晰。
11. 公开合成 Prompt Eval 覆盖合并、拆分、情景、共性、分歧、空 AI、提示注入和输入外推理；确定性 gate 先于语义 Judge，严重或不确定语义失败进入人工复核。
12. 数据库契约、Worker、Protocol、控制面 API、Reader repository/DTO、组件、权限、响应式、端到端、lint、production build、`git diff --check` 和脱敏检查全部通过后，才具备发布资格；生产完成声明还必须通过稳定域名上的已登录只读验收和一次真实新 v5 生成链路验收。

## 11. Spec 自检

本设计只修订跨博主每日判断的阅读语义，没有改变底层采集或单博主证据职责；完整主线、情景分支、按博主操作归因和零到多条 AI 判断均有明确结构与证据边界。AI 综合研判被限定为多来源、输入内的二阶归纳，不进入模块 2 的独立投资决策。历史兼容、失败隔离、Prompt Eval、普通 Reader 安全投影和跨层版本一致性均已明确，范围可以由一个独立 implementation plan 完成。
