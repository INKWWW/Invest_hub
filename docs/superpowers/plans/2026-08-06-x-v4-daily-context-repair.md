...
生产数据库和 Worker 已使用 `v4-x-*` 合同，但 Control Plane 的 `x-daily-judgements` repository 仍按 `v3-x-*` 解析 RPC context，导致合法 v4 context 在 Provider 调用前被拒绝为 `schema_error`。历史失败 run、采集数据和冻结证据保持不可变；修复不得直接 DML、重置 attempt 或重新采集 X。
