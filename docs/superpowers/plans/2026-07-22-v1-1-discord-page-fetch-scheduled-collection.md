...
**Plan status:** 已批准（用户确认 2026-07-22）；**已延后（用户决定 2026-07-22）**。个人账号 Page Fetch 不进入生产，以下任务不再执行。历史批准记录保留；如未来取得频道管理员 Bot 授权，须新建独立 Spec/Plan，不能直接恢复本 Plan。

**Spec:** [V1.1 Discord Page Fetch 定时采集修订 Spec](../specs/2026-07-22-v1-1-discord-page-fetch-scheduled-collection-amendment.md)

## 执行收口说明（2026-07-22）

本项目已完成与 Discord 采集相关方案的评估与受控技术探索。用户决定不以个人 Discord 账号进行生产自动化，原因是账号与合规风险；因此不启用 Page Fetch 定时任务、手动任务、真实验收、部署或代码合并。任何未合并的探索代码均不是可上线实现。

后续仅在频道管理员明确授权 Bot 进入目标频道后，再以新的、独立批准的方案评估 Bot 采集。未来方案至少应定义 Bot 的最小读取权限、手动触发边界、消息内容访问条件、增量 checkpoint、失败恢复和审计证据。
