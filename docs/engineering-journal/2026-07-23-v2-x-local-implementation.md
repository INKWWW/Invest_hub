...
## 2026-07-26 水位完成修复与自动运行验收

一次真实范围在逐页持久化后被范围完成接口拒绝。脱敏生产聚合确认根因不是登录、X 采集、Codex CLI 或页面持久化：已有失败窗口的结束时间均不晚于连续覆盖水位，却仍被前序完整性判断当作未完成工作。

修复后的数据库规则只忽略这类已覆盖的历史失败审计记录；结束时间晚于当前水位的失败窗口仍会阻塞范围完成。新增 pgTAP 回归先复现 `predecessor_range_incomplete`，随后验证当前水位窗口可完成并推进水位。完整本地验证通过：22 个 pgTAP 文件/291 项、Worker 133 项、控制面 135 项、lint 与 production build；脱敏检查和差异检查也通过。

远程迁移应用后，受控真实运行完成当前范围并持久化 `2` 条逐帖分析和 `1` 段每日观点，当前水位无待重试窗口。随后加载 `com.investhub.x-worker` 常驻服务；它已处于运行状态，并成功完成首个自动调度轮次（投递 1 个窗口、无延期来源、成功结束）。线上 `/x` 阅读路由 HTTP 可达。

本节只记录聚合计数、状态和结论；不记录博主、帖子、链接、正文、Prompt、完整模型输出、凭据、设备身份或本地证据路径。V2 仍保持受控生产试运行：真实引用/回复/转发关系与可恢复失败后的恢复尚未完整覆盖。

## 2026-07-26 正式 Reader 绑定复核与验收

后续复核确认，已发布 Reader 别名曾保留过期的数据源环境绑定。已将正式控制面重新绑定到当前生产 Supabase 并重新部署为 Ready；不记录具体环境值、项目标识、账号、帖子、链接、Cookie 或凭据。

用户重新登录后，对生产 `/x` 仅做结构化验收：页面存在 X 阅读主体、Discord/X 切换导航、最新日期选择与证据明细；Reader 显示成功更新，未显示失败或处理中。常驻 X 服务保持运行，最近一次成功调度后的轮询正确返回 `no_task`。完整本地回归重跑为 22 个 pgTAP 文件/291 项、Worker 133 项、控制面 135 项、lint、production build 与脱敏检查通过。

这项复核关闭了发布 Reader 与实际 X 持久化数据源错位的风险，但不改变 V2 的 conditional 结论：真实引用/回复/转发关系和可恢复失败恢复仍待长期观察或新的明确验收。
