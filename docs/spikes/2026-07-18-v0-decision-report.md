...
V0 不交付普通用户正式阅读页、不接入 X、不提供多来源运营、不实现 GLM 或自动回退。真实 Discord 运行必须由管理员明确提供已登录专用浏览器配置档和有权限来源，并在仓库外保存证据；远程部署必须使用隔离的 V0 项目和不含真实生产数据的凭据。只有在真实 Discord 验收补齐，并重新确认 Codex c100/c5/240 秒/最多 3 次运行行为后，才能开始 V1 规格文档。
