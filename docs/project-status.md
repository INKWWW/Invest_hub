...
**X 当日判断延迟结算宽限期（2026-08-03）：** 已部署新 migration `20260802160849`。正常调度器新建的 batch 现在按上海 `natural_date` 统一结算至次日 01:00；已结算的历史 batch、来源覆盖与判断版本均未回写。正式 `/x` 已验证“采集超时，未形成判断”与未纳入来源说明，且未暴露内部排除字段。该宽限期仅缓解本机恢复后的同日补采，不替代保持电脑唤醒、登录态有效或云端常驻能力。
