...
- 普通用户邀请码时长与掩码列表 implementation status：本地实现和网页验收已完成。数据库新增掩码、时长和来源 HMAC 限流边界；普通用户邀请码使用 8 位字母数字规则，Worker 长随机码保持不变；管理员卡片支持每次填写小时数、一次性明码显示、刷新后的掩码和实时倒计时。当前本地验证为 24 个 pgTAP 文件/315 条断言、37 个控制面测试文件/165 项、lint、production build、脱敏检查和实际页面交互通过。目标 Vercel 项目尚未对当前 CLI 身份开放，生产部署与生产网页验收仍待授权；详见 [工程记录](engineering-journal/2026-07-28-user-invite-duration-and-mask.md)。
