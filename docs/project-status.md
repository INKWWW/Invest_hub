...
- `chunk-size 500` 的 500 条真实运行首个 chunk 在 240 秒超时；修复后已能正常回收进程，但该 chunk size 不作为安全配置；500/1000 条完整容量尚未验证，结论为 `unverified`；
...
下一阶段需要在本地受保护环境完成约 500 条和 1000 条 Codex CLI 真实容量运行，优先比较 chunk size 100/250，并完成 P50/P95、失败分类和质量复核，再根据证据更新 Spike-02 结论。
