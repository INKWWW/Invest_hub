...
**投资研究 Agent Feature Contract 与 Delivery Plan（2026-08-11）：** 用户已明确批准 Matt profile 的[完整 Spec](../.scratch/investment-research-agent/spec.md)，并在审阅后明确批准由 17 张 tracer-bullet tickets 及其 blocking edges 构成的完整 Delivery Plan；全部 ticket 使用同一批准日期与证据。获批范围是独立的核心投资研究 Agent：继续使用 Vercel、Supabase 与本机 Codex CLI Worker，Supabase 作为用户隔离的 Research Thread、Agent Run、Quota、Trace 与 Memory 权威存储；首版不接入 X/Discord Agent sourcing，技术画线 Skill 保持并行开发并在交付后走独立集成 Feature Contract。Ticket 02“私有 Research Thread 聊天骨架”已在 `defea96cae3dc98f285cd07a248f8678449836b2` 集成并完成组合验证，Ticket 04“Research Quota 管理与用户余额”已完成本地实现与验证；当前 implementation frontier 为 Ticket 01“Codex CLI 运行合同与安全事件 Spike”和 Ticket 03“投资范围与免额度响应”。全量 `tsc --noEmit` 仍有既有非 Agent 测试类型错误，未将其记为通过；Ticket 17 的 push、remote migration、Vercel deployment、Worker变更、真实生产验收等外部状态操作仍要求独立 Release Authorization。
...
**X 判断对象范围与 Reader 展示 v4（2026-08-05）：** 已按批准的 [Spec](superpowers/specs/2026-08-05-x-judgement-scope-and-reader-presentation-design.md) 与 [Plan](superpowers/plans/2026-08-05-x-judgement-scope-and-reader-presentation.md) 完成 v4 正常链路：单帖、窗口和跨博主输出新增 `action_scope_status` 三态合同；对象未明确时只保留操作表述并在 Reader 显示“对象未明确，不可据此执行”，不再制造伪对象；历史 v3 replay 保持冻结 v3。Reader 已统一“主题 → 观点 NN → 直接陈述 → 元信息”层级，两个大模块使用浅色标题带，博主标题行加粗，安全 DTO 显式携带操作范围字段。提交 `1341af5` 的本地证据为 Supabase 41 files/644 tests、Worker 188 tests、控制面 42 files/239 tests、lint、Turbopack production build、diff check 和 redact check 全部通过。生产 Supabase `invest-hub-v1` 已应用 migration `20260805141108`，远端 history 已确认；控制面 deployment `dpl_4XHChU26dqiYAnmspYFtgyTLLnCK` 已 READY 并切换稳定别名，`com.investhub.x-worker` 已 reload 且 running。已登录 `/x` 标签页可读取生产内容和筛选控件，但浏览器 DOM/滚动接口持续超时，截图只足以确认页面可见，不能把桌面/375px 逐项视觉验收标记为通过；下一次正常 v4 scheduler window 仍是证明生产端到端 v4 持久化的唯一证据。
...
**X 当日判断延迟结算宽限期（2026-08-03）：** 已部署新 migration `20260802160849`。正常调度器新建的 batch 现在按上海 `natural_date` 统一结算至次日 01:00；已结算的历史 batch、来源覆盖与判断版本均未回写。正式 `/x` 已验证“采集超时，未形成判断”与未纳入来源说明，且未暴露内部排除字段。随后补充已完成 batch 的输入覆盖行，明确已纳入、无新增和未纳入数量，以及主题署名只代表直接支持或反对该主题的博主。该宽限期仅缓解本机恢复后的同日补采，不替代保持电脑唤醒、登录态有效或云端常驻能力。
...
**X 跨博主当日判断 Prompt v3 生产发布（2026-08-03）：** 已批准的 [v3 设计](superpowers/specs/2026-08-03-x-daily-judgement-prompt-v3-design.md) 与 [Implementation Plan](superpowers/plans/2026-08-03-x-daily-judgement-prompt-v3.md) 将新生成判断升级为三类可追溯投资信息：个股与产业、市场结构、投资策略与心态。只有博主在输入中明确表达的建仓、买入、卖出等倾向才会以固定枚举、适用范围和条件展示；系统不生成交易建议。版本化公共 Prompt 为 `v3-x-cross-blogger-1`，历史 v2 输出保持可读。生产 migration `20260803100000` 已应用并由远端 history 确认；正式 deployment `dpl_EfqWarB463BqpLjEdeSXZx9nxkbU` 为 Ready，稳定入口 `https://invest-hub-v0-control-plane.vercel.app/x` 已指向提交 `4c15c28`；本机 Worker 已恢复并加载 main。已认证 `/x` 只读验收确认层级、筛选与覆盖说明保持正常。现有 v2 judgement 不会被伪造成第三类内容，下一次正常生成的 v3 judgement 才会展示新栏目和博主倾向。验收为 pgTAP 35 files / 580 tests、全量 Worker 165 tests、控制面 42 files / 232 tests、lint 和默认 production build 全部通过。

**X 跨博主当日判断最终边界本地验收（2026-08-01）：** [初始](superpowers/plans/2026-07-31-x-cross-blogger-daily-judgement-summary.md)、[再生成](superpowers/plans/2026-08-01-x-daily-judgement-regeneration.md)、[最终边界修复](superpowers/plans/2026-08-01-x-daily-judgement-final-remediation.md)和[最终审查修复](superpowers/plans/2026-08-01-x-daily-judgement-final-review-remediation.md)四个 Implementation Plan 均已批准，本地确定性 gate 已全部执行。Reader 仅在存在持久化 judgement version 时公开 `coverageStatus`；collecting/pending/failed 且无 version 时为 `null`，不会伪装成 `no_new_information`。新批次 ensure 仍要求当前 enabled/resolved 来源；已冻结批次的 claim 则按 batch snapshot 中的 X 来源授权及 Worker online、fresh heartbeat、`x_sync` capability 判断，来源或 profile 归档不会使既有队列失联。DB 生命周期只由真实 judgement pgTAP 026/027/028/029 验证，包括 00:00 前一上海自然日、完整来源快照、落后来源 partial、三次租约上限、initial/regeneration 终态区别和 immutable revision 1 → 2；不使用 Python state imitation 证明数据库行为。真实 Worker 聚焦组 58 tests 验证 schema/runtime/protocol/recovery，以及 scheduled CLI 将安全的 `judgement_dispatch_failed` 布尔 telemetry 保留在输出中而不输出调度详情；前置 control-plane 组的实际 `api.integration.test.ts` 覆盖 Worker completion API，去重后的 final Node 组为 8 files / 40 tests，验证 Reader safe output、revision history 与 all/source/date URL 恢复。最终本地证据为 pgTAP 33 files / 564 tests、全量 Worker 161 tests、控制面 42 files / 232 tests 全通过，lint 通过；默认 `npm run build` 仍以 exit 1 失败，因为既有外部 `node_modules` symlink 被 Turbopack 拒绝，独立 `npm run build -- --webpack` 通过 32 个页面，但只算 supplemental evidence。2026-08-02 已在受控对账后完成远端 migration、部署、Worker 重启和 authenticated `/x` 页面验收；详见 [工程记录](engineering-journal/2026-08-01-x-cross-blogger-daily-judgements.md)。

**X 跨博主当日判断生产发布（2026-08-02）：** 先以无副作用 marker 对齐远端既有 `20260731084640` history，再由 dry-run 精确确认十条后续 migration；生产实际应用 `20260731100000` 至 `20260801180000`，随后只读确认全部十一条版本和 `deferred_source_ids` 终态失败隔离语义。control-plane 部署 `dpl_8TVpwmZfW2FnYmJ2zNJQdLV5Hxbb` 为 Ready，稳定入口 `https://invest-hub-v0-control-plane.vercel.app/x` 已指向该 deployment；`com.investhub.x-worker` 已重启并处于 loaded 状态。已认证普通用户在正式 `/x` 完成桌面及 375px 验收：日期降序、判断总结在单博主观点前、每位博主独立分块、博主/日期筛选可见，未发现错误覆盖层或横向溢出。没有手工伪造采集或 Provider 输入，后续 judgement 仍只由正常 X scheduler 领取。
...
- V2 implementation status：已完成受控本地 Collection runtime、远程 migration、生产控制面部署、来源身份解析、连续覆盖初始化、真实持久化范围闭环，以及 `com.investhub.x-worker` 常驻安装与首个自动完成窗口。最近一次受控真实范围持久化了 `2` 条逐帖分析和 `1` 段每日观点；自动轮询随后成功完成一个到期窗口，当前水位无待重试窗口。X Worker 与生产控制面数据库绑定已实测一致，生产 `/x` 路由 HTTP 可达。该阶段记录的回归为 22 个 pgTAP 文件/291 项、Worker 133 项、控制面 135 项、lint、production build 和脱敏检查通过；当前最新的本地 judgement gate 及默认 Turbopack 环境失败见本页顶部 2026-08-01 记录。V2 当前是受控生产试运行；正式退出门槛及限制见 [V2 Final Report](spikes/2026-07-22-v2-x-decision-report.md)。
