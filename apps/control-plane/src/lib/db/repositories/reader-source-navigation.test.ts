...
const databaseMocks = vi.hoisted(() => ({ rows: new Map<string, unknown[]>(), filters: [] as Array<{ table: string; field: string; value: unknown }>, selects: [] as Array<{ table: string; columns: string }> }));
...
describe("X reader date projection", () => {
  beforeEach(() => {
    ...
    databaseMocks.rows.set("x_collection_batch_sources", [
      { batch_id: "batch-16", source_id: "source-a", source_display_name: "Alpha at sixteen", settlement_status: "excluded" },
  ...
  it("selects no unused internal evidence or segment identity fields", async () => {
    ...
    expect(projections).not.toContain("created_at");
  });
